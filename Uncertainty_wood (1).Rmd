...
lines(wvl, log(class_dst_mean), col = scales::alpha(palette()[4],0.8), lwd = 0.6) 
lines(wvl, log(class_dst_mean + class_dst_sd), col = scales::alpha(palette()[4],0.4), lty = 2, lwd = 0.5) 

lines(wvl, log(class_dst_mean - class_dst_sd), col = scales::alpha(palette()[4],0.4), lty = 2, lwd = 0.5)  

lines(wvl, log(class_dsp_mean), col = scales::alpha(palette()[6],0.8), lwd = 0.6) 
lines(wvl, log(class_dsp_mean + class_dsp_sd), col = scales::alpha(palette()[6],0.4), lty = 2, lwd = 0.5) 
lines(wvl, log(class_dsp_mean - class_dsp_sd), col = scales::alpha(palette()[6],0.4), lty = 2, lwd = 0.5) 
...
```{r}
pls_model <- train(y = as.factor(df_center_mean$Bag),
...
```{r}
loc_pred_pls_1 <- predict_with_uncertainty_parallel(df_center[1:15,-c(2)],
                                              pls_model,
                                              simulations = X_simulated[1:5],
                                              )
loc_pred_pls_2 <- predict_with_uncertainty_parallel(df_center[16:30,-c(2)],
                                              pls_model,
                                              simulations = X_simulated[6:10],
                                              )
loc_pred_pls_3 <- predict_with_uncertainty_parallel(df_center[31:36,-c(2)],
                                              pls_model,
                                              simulations = X_simulated[11:12],
                                              )
loc_pred_pls <- rbind(loc_pred_pls_1,
                      loc_pred_pls_2,
                      loc_pred_pls_3)
```
...
color <- palette()[rep(c(4,6), each = 6)]
...
```{r}
global_limits <- predict_noise_confidence_interval(pls_1,
                                                   df_center$Sample,
                                                   n_sim = 500,
...
```{r}
df_train_mean <- aggregate(.~ Class + Sample, data = df_train, FUN = mean)
```

```{r}
df_center_mean <- predict(center_parameters, df_train_mean[,-c(1:2)])
```

```{r}
glob_pred <- classify_sample_with_limits(df_center_mean, pls_1, global_limits$upper)
```

```{r fig.width=12.5, fig.height=2.5, fig.align="center", dpi =900, echo = F}
...
par(mgp = c(1.5, 0.5, 0)) 
par(mar = c(3,3,1,1))
n_samples_class <- as.numeric(table(df_train_mean$Class))

n_samples <- max(n_samples_class)

color <- palette()[rep(2:1, each = 5)]
...
plot(pca$x[,1],
  ...
  points(global_limits[["sim_X"]] %*% as.matrix(pca$rotation[,1]),
         global_limits[["sim_X"]] %*% as.matrix(pca$rotation[,2]),
         pch = 16,
         cex = 0.5,
