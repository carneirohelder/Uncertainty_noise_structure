predict_with_uncertainty_parallel <- function(X, model, n_sim = 30, ncores = parallel::detectCores() - 1, return_simulations = FALSE) {
  ...
  results <- future.apply::future_lapply(sample_ids, function(id) {
    ...
    sim_X <- MASS::mvrnorm(n = n_sim, mu = x_bar, Sigma = Sigma)
  ...
  # If simulations are included, keep list output (no rbind)
  if (return_simulations) {
    return(results)  # list with per-sample elements including sim_X
  } else {
