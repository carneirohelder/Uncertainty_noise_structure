...
```{r}
# plot(log(class_ora_var),
#      log(class_ora_mean),
#      type = 'p')
```
```{r}
# cor_matrix <- (cov(class_ora_cent))
```

```{r}
# n_colors <- 100
# color_fn <- colorRampPalette(c("blue", "white", "red"))
# 
# # Flatten the correlation matrix and scale it to color indices
# z_vals <- cor_matrix
# z_scaled <- cut(z_vals, breaks = n_colors)
# colors <- color_fn(n_colors)[z_scaled]
# 
# # Draw the 3D surface
# persp3d(
#   x = 1:nrow(z_vals),
